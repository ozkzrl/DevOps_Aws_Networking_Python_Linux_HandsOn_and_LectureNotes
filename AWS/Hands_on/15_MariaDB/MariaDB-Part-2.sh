...
# Create new database named 'ondiadb'.
CREATE DATABASE ondiadb;

# Show newly created database.
SHOW DATABASES;

# Create a user named 'ondiauser'.
CREATE USER ondiauser IDENTIFIED BY 'ondia1234';

# Grant permissions to the user ondiauser for database ondiadb.
GRANT ALL ON ondiadb.* TO ondiauser IDENTIFIED BY 'ondia1234' WITH GRANT OPTION;
